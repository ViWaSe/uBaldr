...
class Client:
    ...
    def __init__(
        ...
        ------
            ...
            3. To disconnect, call wifi.disconnect()
        """
        ...
        self._led_flash(250, 250)
        self._led_flash(250, 250)
        self._led_flash(1000, 0)
    ...
    def connect(self, timeout=3):
        ...
        for attempts in range(1, max_attempts + 1):
            ...
            if self._wait_connection(timeout=timeout):
                self.led_onboard.on()
                ifconfig = self.wlan.ifconfig()
                self.event.log('I',f'Connected successfully to {self.wlanSSID}. IP = {ifconfig[0]}')
                self._save_ip(ifconfig[0])
                return
            else:
                wstat = self._error_handler(self.wlan.status())
    ...
    def _wait_connection(self, timeout=10):
        """
        ...
        while True:
            ...
            if status == 0 or status == 1000:  
                ...
                return False
            self._led_flash(250, 500)
            self._led_flash(250, 0)
    ...
    def _led_flash(self, on=1000, off=0):
        self.led_onboard.on()
        time.sleep_ms(on)
        self.led_onboard.off()
        time.sleep_ms(off)

    def _error_handler(self, errno):
        ERROR_CODES = {
            0: 'LINK_DOWN',
            1: 'LINK_JOIN',
            2: 'LINK_NOIP',
